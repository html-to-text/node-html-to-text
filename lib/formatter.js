...
const { numberToLetterSequence, numberToRoman, wordwrap } = require('./helper');
...
/**
 * Convert an inline-level element to a plain string.
 *
 * @type { FormatCallback }
 */
function formatInline (elem, fn, options) {
  return fn(elem.children, options);
}

/**
 * Convert a block-level container to a plain string.
 *
 * @type { FormatCallback }
 */
function formatBlock (elem, fn, options) {
  const result = fn(elem.children, options);
...
function formatHorizontalLine (elem, fn, options) {
  return '\n' + '-'.repeat(options.wordwrap || 40) + '\n\n';
}

/**
 * @param   { { node: DomNode, prefix: string }[] } items        List items with their prefixes.
 * @param   { RecursiveCallback }                   fn           Recursive callback to process child nodes.
 * @param   { Options }                             options      HtmlToText options.
 * @param   { boolean }                             isNestedList Whether this list is nested and needs extra spacing.
 * @returns { string }
 */
function formatList (items, fn, options, isNestedList) {
  let result = isNestedList ? '\n' : '';
  if (items.length) {

    const prefixLength = items[0].prefix.length;
    const innerOptions = Object.assign({}, options);
    if (innerOptions.wordwrap) {
      // Reduce the wordwrap for sub elements.
      innerOptions.wordwrap -= prefixLength;
    }
    const spacing = '\n' + ' '.repeat(prefixLength);
    for (const item of items) {

      innerOptions.lineCharCount = 0;
      // Each line within list item is aligned to the first line with prefix.
      const text = fn(item.node.children, innerOptions).replace(/\n/g, spacing);
      result += item.prefix + text + '\n';
...
/**
 * @param   { DomNode }   elem Element to get children from.
 * @returns { DomNode[] }
 */
function getNonWhiteSpaceChildren (elem) {
  return (elem.children || []).filter(function (child) {
...
function formatUnorderedList (elem, fn, options) {
  ...
  const items = getNonWhiteSpaceChildren(elem)
    .map(node => ({ node: node, prefix: prefix }));
  return formatList(items, fn, options, isNestedList);
}
...
function formatOrderedList (elem, fn, options) {
  const isNestedList = get(elem, 'parent.name') === 'li';
  const nestedSpacing = (isNestedList ? '' : ' ');
  const start = Number(elem.attribs.start || '1');
  const indexFunction = getOrderedListIndexFunction(elem.attribs.type);
  // With Roman numbers, index length is not as straightforward as with Arabic numbers or letters,
  // so the dumb length comparison is the most robust way to get the correct value.
  let maxIndexLength = 0;
  const itemsWithIndices = getNonWhiteSpaceChildren(elem)
    .map(function (node, i) {
      const index = indexFunction(start, i);
      if (index.length > maxIndexLength) {
        maxIndexLength = index.length;
      }
      return { node: node, index: index };
    });
  const items = itemsWithIndices.map(x => ({
    node: x.node,
    prefix: nestedSpacing + x.index + '. ' + ' '.repeat(maxIndexLength - x.index.length)
  }));
  return formatList(items, fn, options, isNestedList);
}
 ...
 * @returns { (start: number, i: number) => string }
 */
function getOrderedListIndexFunction (olType = '1') {
  switch (olType) {
    case 'a': return function (start, i) { return numberToLetterSequence(start + i, 'a'); };
    case 'A': return function (start, i) { return numberToLetterSequence(start + i, 'A'); };
    case 'i': return function (start, i) { return numberToRoman(start + i).toLowerCase(); };
    case 'I': return function (start, i) { return numberToRoman(start + i); };
    case '1':
    default: return function (start, i) { return (start + i).toString(); };
  }
}
