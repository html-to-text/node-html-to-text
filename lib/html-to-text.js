...
function getClasses(elem) {
  return elem.attribs && elem.attribs.class ? elem.attribs.class.split(" ") : [];
}

function getIds(elem) {
  return elem.attribs && elem.attribs.id ? elem.attribs.id.split(" ") : [];
}

function shouldElementIgnored(elem, ignoreSelectors) {
  return ignoreSelectors.some(function (selector) {
    var parsedSelector = helper.parseSelector(selector);

    switch (parsedSelector.type) {
      case 'class': {
        var classes = getClasses(elem);

        return classes.includes(parsedSelector.value);
      }
      case 'id': {
        if (!elem.attribs || !elem.attribs.id) {
          return false;
        }

        return elem.attribs.id === parsedSelector.value;
      }
      default: {
        return elem.name.toLowerCase() === parsedSelector.value;
      }
    }
  });
}
  ...
  function walk(dom) {
    ...
    dom.forEach(function(elem) {
      ...
      if (elem.name === splitTag.element) {
        var documentClasses = getClasses(elem);
        var documentIds = getIds(elem);
...
function walk(dom, options, result) {
  ...
  dom.forEach(function(elem) {
    switch(elem.type) {
      case 'tag':
        if (shouldElementIgnored(elem, options.ignoredSelectors)) {
          return;
        }
