...
function htmlToText(html, options) {
	...
	var result = walk(filterBody(handler.dom, options), options);
	return _s.strip(result);
}

function filterBody(dom, options) {
	var result = null;
	...
	var element = elementRe.exec(options.baseElement)[1];
	var classes = getParams( /\.([\d\w-]*)/g, options.baseElement);
	var ids = getParams( /#([\d\w-]*)/g, options.baseElement);

	function walk(dom) {
		if (result) return;
		_.each(dom, function(elem) {
			if (elem.name === element) {
				var documentClasses = elem.attribs.class ? elem.attribs.class.split(" ") : [];
				var documentIds = elem.attribs.id ? elem.attribs.id.split(" ") : [];

				if ((classes.every(function (val) { return documentClasses.indexOf(val) >= 0 })) &&
					(ids.every(function (val) { return documentIds.indexOf(val) >= 0 }))) {
